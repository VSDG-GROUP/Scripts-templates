        ...
        with open(username+'.csv', 'w', newline='', encoding='utf-8') as file:
            ...
            for post in posts:
               ...
               # print("post location: "+str(post.location))
                ...
                #writer.writerow(["post",post.mediaid, post.profile, post.caption, post.date, post.location, posturpostasasasasl,  post.typename, post.mediacount, post.caption_hashtags, post.caption_mentions, post.tagged_users, post.likes, post.comments,  post.title,  post.url ])
                writer.writerow(["post", post.location, posturl, post.url])
...
if __name__=="__main__":
    ...
    if cm == 'user':
        cls.download_users_profile_picture(user)
        cls.download_users_posts_with_periods(user)
    if cm == 'tags':
        cls.download_hastag_posts(user)
        cls.get_users_followers(user)
    
        #cls.get_users_followings("best_gadgets_2030")
        #cls.get_post_comments("laydline")
    if cm == 'post':
        cls.get_post_info_csv(user)